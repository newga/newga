...
Yii::import('application.extensions.cimage.Image_Driver');
...
class CImage {
	...
	public function __construct($image, $config = NULL)
	{
        ...
        // Load the driver
        Yii::import("application.extensions.cimage.drivers.$driver");
	...
	public function shrink($width, $height, $master)
	{
	...
	public function resizeForCrop($width, $height)
	{
		// First if the required width or height is more than the actual width or height
		if(0)//$this->image['width'] < $width || $this->image['height'] < $height)
		{
			throw new CException('image ('.$this->image['width'] .'x'. $this->image['height'].') is too small to resize to '.$width.'x'.$height);
		}
		else
		{
			// get the ratios
	...
	{
		// Resizes the image by changing the shortest side to $width or $height
		// Then crops an image to the other dimension
		$ratio = $width / $height;
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		if ( ! is_writable($dir))
			throw new CException('image directory ('.$dir.') was unwritable');
